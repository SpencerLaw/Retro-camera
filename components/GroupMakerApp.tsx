...
export const GroupMakerApp: React.FC = () => {
  ...
  const animate = () => {
    setBalls(prev => prev.map(ball => {
      ...
      let nvx = ball.vx;
      let nvy = ball.vy;
      if (nx < 10 || nx > 350) nvx *= -0.9;
      if (ny < 40 || ny > 450) nvy *= -0.9;
      nvy -= 0.08; if (ny < 120) nvy += 0.25;
      return { ...ball, x: nx, y: ny, vx: nvx, vy: nvy, angle: ball.angle + nvx * 4 };
    }));
  ...
  const handleStartGrouping = async () => {
    ...
    for (let i = 0; i < shuffledNames.length; i++) {
      ...
      setClawState({ height: 350, isGrabbing: false });
      await new Promise(r => setTimeout(r, 450));
      setClawState({ height: 350, isGrabbing: true });
  ...
  return (
    <div className="group-maker-app">
      <div className="group-maker-container">
        ...
        <div className="factory-panel left-panel">
          ...
          <div className="input-content">
            ...
            <div className="clear-btn-wrap">
              <button onClick={() => setNames("")} className="start-btn-arcade" style={{fontSize: '1.5rem', padding: '15px'}}>{t('home.groupMaker.clearBtn')}</button>
            </div>
          </div>
        </div>
        ...
        <div className="factory-panel middle-panel">
          ...
          <div className="machine-controls">
            <div className="flex justify-between items-center bg-white/10 p-4 rounded-xl border border-white/20 mb-4">
              <span className="font-black text-white text-2xl">{t('home.groupMaker.groupCount')}</span>
              <input type="number" className="w-24 p-2 rounded-xl text-center font-black text-blue-900 text-3xl outline-none" value={numGroups} onChange={(e) => setNumGroups(parseInt(e.target.value) || 2)} disabled={isAnimating} />
            </div>
      ...
      <button onClick={() => navigate('/')} className="home-back-btn shadow-2xl hover:scale-110 active:scale-90 transition-all"><ArrowLeft size={45} strokeWidth={4} /></button>
    </div>
  );
};