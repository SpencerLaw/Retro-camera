        ...
        view === 'week' ? (
          <div className="kawaii-card p-10 bg-white/40 border-white shadow-2xl">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                <h3 className="font-candy text-4xl text-[#5D4037] tracking-tight">七日梦想航志</h3>
              ...
              {weekDays.map((day, idx) => (
                <div key={day} className="kawaii-card bg-white p-6 flex justify-between items-center group cursor-pointer hover:bg-[#FFF9C4]/30 border-white shadow-lg transition-all hover:translate-x-3">
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <div className="text-[12px] font-bold text-[#5D4037] opacity-30 uppercase tracking-[0.3em]">{day}</div>
                      <div className="font-bold text-[#5D4037] text-xl">
                        {idx === 0 ? '重点：奇幻作业挑战' : idx === 6 ? '重点：梦幻岛远足' : '常规探索日'}
                      </div>
                    </div>
                  </div>
                  <ChevronRight size={22} className="text-[#5D4037] opacity-10 group-hover:opacity-100 group-hover:text-[#FF8095] transition-all" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="kawaii-card p-12 bg-white/40 border-white shadow-2xl relative overflow-hidden">
            <div className="flex items-center gap-4 mb-10">
              <div className="w-14 h-14 bg-[#FFB6C1] rounded-[28px] flex items-center justify-center shadow-xl border-4 border-white animate-float-kawaii">
                <Map size={28} className="text-[#5D4037]" />
              </div>
              <div>
                <h3 className="font-candy text-4xl text-[#5D4037] tracking-tight">梦想进阶航线</h3>
            ...
            <div className="absolute top-32 bottom-20 left-[67px] w-3 bg-[#FF8095]/20 rounded-full shadow-inner"></div>
              ...
              {[
                ...
                <div key={idx} className="flex gap-8 items-start relative z-10">
                  <div className={`w-16 h-16 kawaii-card ${idx === 0 ? milestone.color : 'bg-white'} flex items-center justify-center shrink-0 shadow-2xl border-4 border-white animate-float-kawaii translate-x-[-4px]`} style={{ backgroundColor: idx === 0 ? '' : milestone.color as string, animationDelay: `${idx * 0.3}s` }}>
                    <div className="text-[#5D4037]">{milestone.icon}</div>
                  </div>
                  <div className="pt-3 space-y-1">
                    <h4 className="font-candy text-2xl text-[#5D4037]">{milestone.title}</h4>
