...
- 参考 beui `shader-background`：星芽奇境的星空、雾带、山丘纹理默认保持固定，避免教室白板出现整屏闪烁；只有能量球、根系传递和树冠吸收反馈参与持续动画。
- 每个粒子必须可被下一帧重定向或自然结束，不排队阻塞输入。
- Canvas 循环只在早读监听期间运行；模式切换不会额外创建第二个 `requestAnimationFrame`。

## 7. Depth & Surface

- 星空由连续钴蓝纵向色阶、多层手绘水彩雾带、确定性星尘与少量十字星构成；中段不得出现可见横向断带，不使用大位图或 WebGL shader。
