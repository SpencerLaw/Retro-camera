      ...
      <div className="flex gap-4">
        {(['week', 'future'] as const).map(v => (
          <button
            key={v}
            onClick={() => setView(v)}
            className={`flex-1 py-3 kawaii-button font-candy text-lg transition-all ${view === v ? 'bg-pastel-purple text-white shadow-lg scale-105' : 'bg-white/60 text-macaron/40'}`}
          >
            {v === 'week' ? '本周图纸' : '梦想航线'}
          </button>
        ))}
      ...
      {view === 'week' ? (
        <div className="kawaii-card p-6 bg-white/60">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 bg-pastel-blue rounded-[15px] flex items-center justify-center shadow-sm">
              <Calendar size={20} className="text-white" />
            </div>
            <div>
              <h3 className="font-candy text-2xl text-macaron">七日成长周刊</h3>
              <p className="text-[8px] font-bold text-macaron opacity-30 uppercase tracking-widest">Weekly Growth Map</p>
            </div>
          </div>
            ...
            {weekDays.map((day, idx) => (
              <div key={day} className="kawaii-card bg-white p-4 flex justify-between items-center group cursor-pointer hover:bg-pastel-yellow/30 border-none shadow-sm transition-all hover:translate-x-1">
                <div className="flex items-center gap-4">
                  <div className={`w-10 h-10 rounded-[15px] flex items-center justify-center font-candy text-white shadow-sm ${idx < 3 ? 'bg-pastel-pink' : 'bg-pastel-yellow'}`}>
                    {idx + 1}
                  </div>
                  <div>
                    <div className="text-[10px] font-bold text-macaron opacity-30 uppercase tracking-widest">{day}</div>
                    <div className="font-bold text-macaron text-sm">
        ...
        <div className="kawaii-card p-8 bg-white/60 relative overflow-hidden">
          <div className="flex items-center gap-3 mb-8">
            <div className="w-10 h-10 bg-pastel-pink rounded-[15px] flex items-center justify-center shadow-sm">
              <Map size={20} className="text-white" />
            </div>
            <div>
              <h3 className="font-candy text-2xl text-macaron">梦想进阶航线</h3>
              <p className="text-[8px] font-bold text-macaron opacity-30 uppercase tracking-widest">Future Roadmap</p>
            </div>
          </div>

          {/* Connection Line */}
          <div className="absolute top-24 bottom-16 left-[51px] w-2 bg-macaron/5 rounded-full"></div>
          ...
          <div className="space-y-12 relative">
            {[
              ...
              <div key={idx} className="flex gap-6 items-start relative z-10">
                ...
                <div className="pt-2">
                  <h4 className="font-candy text-lg text-macaron">{milestone.title}</h4>
                  <p className="text-[10px] text-macaron/60 mt-1 font-bold leading-relaxed">{milestone.detail}</p>
                </div>
              </div>
            ))}
          </div>
