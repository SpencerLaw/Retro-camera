...
runTest('imported and selected word banks download pronunciation audio into local cache', () => {
  ...
  assert.match(source, /failedWords\?: string\[\]/);
  assert.match(source, /const getWordAudioFailedListLabel = /);
  ...
  assert.match(source, /暂无真实音频/);
  assert.match(source, /暂无音频：/);
  ...
  assert.match(source, /const missingWords = await getMissingCachedWordAudioKeys\(audioTargets\)/);
  assert.match(source, /if \(missingWords\.length === 0\)/);
  assert.match(source, /completed: audioTargets\.length, percent: 100, done: true, failedWords: \[\]/);
  ...
  assert.match(source, /const failedWords: string\[\] = \[\]/);
  assert.match(source, /const targetLabelByKey = new Map/);
  assert.match(source, /onProgress\?\.\(\{ total: audioTargets\.length, completed: audioTargets\.length - missingWords\.length, percent: 0, done: false, failedWords \}\)/);
  ...
  assert.match(source, /const cached = await cacheDictionaryWordAudio\(word\)/);
  assert.match(source, /if \(!cached\) failedWords\.push/);
  assert.match(source, /onProgress\?\.\(\{ total: audioTargets\.length, completed, percent: visiblePercent, done: false, failedWords: \[\.\.\.failedWords\] \}\)/);
  assert.match(source, /onProgress\?\.\(\{ total: audioTargets\.length, completed: audioTargets\.length, percent: 100, done: true, failedWords: \[\.\.\.failedWords\] \}\)/);
  ...
  assert.match(source, /const bankAudioProgress = wordAudioDownloadProgressByBank\[bank\.id\] \|\| null/);
  assert.match(source, /const bankAudioDownloadLabel = getWordAudioDownloadLabel\(bankAudioProgress\)/);
  assert.match(source, /const bankAudioFailedListLabel = getWordAudioFailedListLabel\(wordAudioDownloadProgressByBank\[bank\.id\] \|\| null\)/);
  assert.match(source, /bankAudioDownloadLabel &&/);
  assert.match(source, /bankAudioFailedListLabel &&/);
  assert.match(source, /bankAudioProgress\?\.done/);
