...
// Added Gift to the imports
import { Calendar, ChevronRight, Sparkles, Map, Flag, Gift } from 'lucide-react';
import { getAiPlanningAdvice } from '../services/gemini';
...
const Planner: React.FC<PlannerProps> = ({ child, role }) => {
  ...
  const handleAiPlan = async () => {
    ...
    const advice = await getAiPlanningAdvice(child.name, 8, "希望能平衡学习和运动，特别是要养成早睡早起的习惯。");
    alert("AI 建议：\n" + advice);
    ...
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
      ...
      {role === 'parent' && (
        <button 
          ...
          className="w-full sketch-button bg-white py-4 flex items-center justify-center gap-2 font-handwriting text-xl text-indigo-600 hover:bg-indigo-50"
        >
          {isGenerating ? 'AI 正忙着画画...' : <><Sparkles size={20} /> 让 AI 帮我写规划</>}
        </button>
      )}
    </div>
  );
