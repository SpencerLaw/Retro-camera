...
export default function App() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (savedData && JSON.parse(savedData).teachers?.length >= 100) {
                    ...
                    {Array.from({ length: 12 }, (_, i) => i + 1).map(classNum => {
                      ...
                      return (
                        <tr key={classNum} className="hover:bg-slate-50/50 transition-colors text-center">
                          ...
                          <td className="p-3 border-r border-slate-100 font-bold text-indigo-700 bg-indigo-50/20 text-center">{getAdviserName(selectedGrade, classNum)}</td>
                          <td className="p-3 border-r border-slate-100 text-slate-700">{getSubjectTeacher('语文')}</td>
                          <td className="p-3 border-r border-slate-100 text-slate-700">{getSubjectTeacher('数学')}</td>
                          <td className="p-3 border-r border-slate-100 text-slate-700">{getSubjectTeacher('英语')}</td>
                          <td className="p-3 border-r border-slate-100 text-slate-700">{getSubjectTeacher('物理')}</td>
                          <td className="p-3 border-r border-slate-100 text-slate-700">{getSubjectTeacher('化学')}</td>
                          <td className="p-3 border-r border-slate-100 text-slate-700">{getSubjectTeacher('生物')}</td>
                          <td className="p-3 border-r border-slate-100 text-slate-700">{getSubjectTeacher('政治')}</td>
                          <td className="p-3 border-r border-slate-100 text-slate-700">{getSubjectTeacher('历史')}</td>
                          <td className="p-3 border-r border-slate-100 text-slate-700">{getSubjectTeacher('地理')}</td>
                          <td className="p-3 border-r border-slate-100 text-slate-700">{getSubjectTeacher('体育')}</td>
                          <td className="p-3 text-slate-700 text-center">{getSubjectTeacher('通用')}</td>
                        </tr>
                      );
                    })}
                  </tbody>
