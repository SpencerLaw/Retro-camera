...
dayMap.forEach(d => {
  ...
  for (let r = startRow; r <= endRow; r++) {
    ...
    for (let cNum = 1; cNum <= 12; cNum++) {
      ...
      if (cellVal) {
        rawTimetableItems.push(cellVal);
      }
    }
  }
});

const uniqueAbbrevs = [...new Set(rawTimetableItems)].sort();
console.log('--- Timetable Abbreviations Matching Results ---');
uniqueAbbrevs.forEach(abbrev => {
  if (abbrev === '通用') {
    console.log(`[通用] -> 匹配到: 谢怡 (通用技术)`);
    return;
  }
  if (abbrev === '英程') {
    console.log(`[英程] -> 匹配到: 张红旗 (英语课程/分层组)`);
    return;
  }
  ...
  if (subjName) {
    // Find all matching teachers who actually teach this subject to ensure uniqueness
    const matches = uniqueG2Teachers.filter(tName => {
      if (!tName.includes(namePart)) return false;
      // Get teacher object
      const tObj = Array.from(g2Teachers).map(name => {
        // Find in sheet rows
        ...
        for (let r = 3; r < data.length; r++) {
          ...
          if (!row || !row[classCol]) continue;
          subjectFields.forEach(field => {
            if (row[field.nameCol]?.toString().trim().replace(/\s+/g, '') === name) {
              subjects.add(field.subjectName);
            }
          });
        }
        return { name, subjects: Array.from(subjects) };
      }).find(x => x.name === tName);
      
      return tObj && tObj.subjects.includes(subjName);
    });

    if (matches.length === 1) {
      console.log(`[${abbrev}] -> 匹配到: ${matches[0]} (${subjName})`);
    } else if (matches.length > 1) {
      console.log(`[WARNING] [${abbrev}] -> 匹配到多个老师: [${matches.join(', ')}] (${subjName})`);
    } else {
      console.log(`[ERROR] [${abbrev}] -> 未匹配到任何老师 (${subjName})`);
    }
  } else {
    console.log(`[ERROR] [${abbrev}] -> 未知学科前缀 "${subjChar}"`);
  }
});
